      ...
      <SafeAreaView style={styles.modal}>
        ...
        <ScrollView contentContainerStyle={styles.subscriptionContent} showsVerticalScrollIndicator={false}>
          <View style={styles.subscriptionHero}>
            <View style={styles.subscriptionHeroIcon}><Ionicons name="sparkles" size={24} color="#0D6A49" /></View>
          ...
          <View style={[styles.subscriptionPlanCard, !isPro && styles.subscriptionPlanCardActive]}>
            <View style={styles.subscriptionPlanHeader}>
              ...
              {!isPro ? <View style={styles.subscriptionSelected}><Ionicons name="checkmark" size={14} color="#fff" /></View> : null}
            </View>
            {[
              ...
              "1 estabelecimento",
            ].map((item) => <View key={item} style={styles.subscriptionFeature}><Ionicons name="checkmark-circle" size={17} color="#278657" /><Text style={styles.subscriptionFeatureText}>{item}</Text></View>)}
          </View>

          <View style={[styles.subscriptionPlanCard, styles.subscriptionPlanCardPro, isPro && styles.subscriptionPlanCardActivePro]}>
            <View style={styles.subscriptionProBadge}><Ionicons name="sparkles" size={13} color="#765514" /><Text style={styles.subscriptionProBadgeText}>RECOMENDADO PARA CRESCER</Text></View>
            <View style={styles.subscriptionPlanHeader}>
              ...
              {isPro ? <View style={styles.subscriptionSelectedPro}><Ionicons name="checkmark" size={14} color="#fff" /></View> : null}
            </View>
            {["Produtos e cadastros ilimitados", "Até 5 estabelecimentos", "Relatórios avançados e exportação", "Notificações inteligentes", "Suporte prioritário"].map((item) => <View key={item} style={styles.subscriptionFeature}><Ionicons name="checkmark-circle" size={17} color="#E76832" /><Text style={styles.subscriptionFeatureText}>{item}</Text></View>)}
            <Pressable style={({ pressed }) => [styles.subscriptionCta, pressed && { opacity: 0.82 }]} onPress={activatePro} disabled={isPro} accessibilityRole="button">
              ...
              <Ionicons name={isPro ? "checkmark" : "arrow-forward"} size={17} color="#fff" />
            </Pressable>
          </View>
